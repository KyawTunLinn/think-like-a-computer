# Object Oriented Programming (OOP)

OOP ဆိုစွာ ပရိုဂရမ်းမင်းကို ရေးသားရာမှာ လက်တွေ့လောက (Real World) က အရာဝတ္ထုတိ (Objects) ကို အခြေခံပြီး စဉ်းစားရေ နည်းလမ်းတစ်ခု ဖြစ်ပါရေ။

## ဇာဖြစ်လို့ OOP ကို သုံးရစွာလဲ?
- ကုဒ်တိကို ပြန်သုံးဖို့ လွယ်ကူစေခြင်း (Reusability)
- ပြင်ဆင်ရ လွယ်ကူခြင်း (Maintainability)
- ရှုပ်ထွေးရေ ပြဿနာတိကို ရှင်းလင်းစွာ ဖြေရှင်းနိုင်ခြင်း

## သင်ခန်းစာတိ
### [Chapter 1: Classes & Objects (အခြေခံ သဘောတရားများ)](chapter_1_basics.md)
Blueprint နန့် အိမ်ဆောက်သလို၊ Class နန့် Object တိကို နားလည်ခြင်း။

### [Chapter 2: Inheritance & Polymorphism](chapter_2_inheritance_polymorphism.md)
မျိုးရိုးလိုက်ခြင်းနန့် ပုံစံအမျိုးမျိုး ပြောင်းလဲလုပ်ဆောင်ခြင်း။

### [Chapter 3: Encapsulation & Abstraction](chapter_3_encapsulation_abstraction.md)
အချက်အလက်ကို လုံခြုံစွာ သိမ်းဆည်းခြင်းနန့် မလိုရေ အစိတ်အပိုင်းတိကို ဖျောက်ထားခြင်း။

[Back to Main Menu >](../menu.md)
